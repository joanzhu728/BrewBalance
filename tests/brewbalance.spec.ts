...
test.describe('BrewBalance App', () => {
    ...
    test('should navigate between tabs', async ({ page }) => {
        ...
        await expect(page.locator('[data-testid="nav-dashboard"] .relative')).toHaveClass(/text-amber-400/);

        // Navigate to Balance (Calendar)
        await page.locator('[data-testid="nav-calendar"]').click();
        await expect(page.locator('[data-testid="nav-calendar"] .relative')).toHaveClass(/text-amber-400/);

        // Navigate to History
        await page.locator('[data-testid="nav-history"]').click();
        await expect(page.locator('[data-testid="nav-history"] .relative')).toHaveClass(/text-amber-400/);

        // Navigate to Settings
        await page.locator('[data-testid="nav-settings"]').click();
        await expect(page.locator('[data-testid="nav-settings"] .relative')).toHaveClass(/text-amber-400/);
